...
-- Lock Frame
function core:SetLockedStatus(lock)
	if (lock) then
...
-- SetEnabled
function core:SetEnabledStatus()
	if (cfg.enabled) then
...
core:SetScript("OnEvent",function(self,event,...) if (self[event]) then self[event](self,event,...); else self:RequestUpdate(); end end);
...
SlashCmdList[modName] = function(cmd)
	...
	elseif (param1 == "lock") then
		core:SetLockedStatus(core:IsMouseEnabled());
		AzMsg("|2"..modName.."|r Button frame is now |1"..(core:IsMouseEnabled() and "unlocked" or "locked").."|r.");
	-- Reset
	...
	-- Invalid or No Command
		...
		AzMsg(" |2add <itemlink>|r = Show all items again, by clearing the ignore list");
